...
| BCQ        | 连续/离散 | offline      | Actor-Critic架构，外加VAE网络和扰动网络       | 第一个离线RL算法，完全不需要与环境交互。可以有效避免外推误差，样本利用率高，支持连续和离散动作空间。劣势是实现比较复杂，不能应对奖励稀疏的场景，推理效率低，计算复杂 | BCQ通过限制动作落在离线数据的分布内，强制 agent 行为尽量接近批数据中的行为。<br />VAE网络负责输出在状态s下的动作的分布，使得采样动作在离线数据的分布范围内（通过有监督学习训练VAE获得这个能力），扰动网络（actor）对每个采样得到的动作施加扰动，得到扰动后的动作actions，Q网络对（state，actions）评估价值，取价值最大的动作作为最终动作。Q 网络训练本质上就是“基于离线数据的 TD 学习” | N         |
...
| 算法名        | 动作空间  | on/off-policy | 架构                                                         | 优劣势                                                       | 主要思路                                                     | 探索方法       |
| ------------- | --------- | ------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | -------------- |
| HIRO          | 离散/连续 | off           | 取决于搭载的基础算法                                         | 设计通用、样本效率高，能应对复杂或奖励系数的场景；劣势：要求状态空间必须是可加减的，使得子目标的语义有限，子目标重新标定的计算量巨大 | 两层策略，使用（阶段性状态 减去 起始状态）作为子目标，off-policy方式同时训练两层策略，并使用概率分数高的子目标修正replaybuffer中的经验数据 | 取决于基础算法 |
...
| Option-Critic | 离散/连续 | on / off      | 两层架构，插件化，高层可以是策略网络或者价值网络，选择option，低层可以是PPO或者TD3这样的算法 | 不需要手工定义选项的目标或子任务，算法可以自动学出有意义的高层动作；适用于离散和连续状态/动作空间，插件化；**损失函数等公式很复杂很费解，我看不懂，不推荐。** | 一个option相当于一个工具包，由策略网络、终止函数、Q价值网络构成。高层策略每次选择一个option，根据其策略网络选择动作与环境互动，终止函数决定要不要结束该option，如果终止高层就重新选择option | 取决于基础算法 |

#### 多智能体

| 算法名 | 动作空间  | on/off-policy | 架构                   | 优劣势                 | 主要思路                                                     | 探索方法        |
| ------ | --------- | ------------- | ---------------------- | ---------------------- | ------------------------------------------------------------ | --------------- |
| MADDPG | 连续      | off           | actor-critic，基于DDPG | 合作和竞争场景都支持。 | 基于DDPG，每个agent独立的actor/critic（Q不是V）网络。训练的时候critic的输入为全部agent的观测和动作。还提到了数据不可得的时候通过拟合其他agent的actor的方法、集合子策略提升泛化能力的方法 | actor的动作抽样 |
| COMA   | 离散/连续 | on            | actor-critic           | 仅合作式               | 所有agent共享一套actor-critic网络；训练时候，全局的信息可以用于critic输入以帮助训练；COMA的Q价值函数的设计非常巧妙，可以输入其他所有agent的动作、全局状态、目标agent的id信息，输出目标agent的所有动作的价值，据此集合目标agent的动作分布，就可以计算目标agent的动作的价值期望，再用具体某动作的价值减去价值期望，就是该具体动作的优势，用于策略梯度更新。这就是反事实用于credit assign。 | actor的动作抽样 |
| QMIX   | 离散      | off           | DQN                    | 仅合作式               | 类似VDN，不只是简单的加和关系，还支持非线性的关系，但全局Q函数对agent Q函数式单调递增的。通过构建Mixing网络实现对 agent Q值非线性组合为全局Q值，然后构建td error作为损失函数，隐式的更新agent Q函数。 | epsilon随机     |
| VDN    | 离散      | off           | DQN                    | 仅合作式简单           | 合作式的奖励为全局奖励的场景。思路是 构建全局的Q函数，它等于每个agent Q函数的加和，通过全局Q函数和全局奖励值构建TD Error作为损失函数，从而隐式的更新agent Q函数，达到训练和协同的目的。 | epsilon随机     |
