...
| 算法名 | 动作空间  | on/off-policy | 架构         | 优劣势               | 主要思路                                                     | 探索方法        |
| ------ | --------- | ------------- | ------------ | -------------------- | ------------------------------------------------------------ | --------------- |
| MADDPG | 连续      | off           | actor-critic | 基于DDPG好理解好实现 | 基于DDPG，每个agent独立的actor/critic（Q不是V）网络。训练的时候critic的输入为全部agent的观测和动作。还提高到了数据不可得的时候通过拟合其他agent的actor的方法、集合子策略提升泛化能力的方法 | actor的动作抽样 |
| COMA   | 离散/连续 | on            | actor-critic |                      | 所有agent共享一套actor-critic网络；训练时候，全局的信息可以用于critic输入以帮助训练；COMA的Q价值函数的设计非常巧妙，可以输入其他所有agent的动作、全局状态、目标agent的id信息，输出目标agent的所有动作的价值，据此集合目标agent的动作分布，就可以计算目标agent的动作的价值期望，再用具体某动作的价值减去价值期望，就是该具体动作的优势，用于策略梯度更新。这就是反事实用于credit assign。 | actor的动作抽样 |
|        |           |               |              |                      |                                                              |                 |
