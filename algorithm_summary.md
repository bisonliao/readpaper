...
| RND       | 离散/连续           | 取决于搭载的基础算法 | 基于但不限于 Actor-Critic，RND是附加的通用模块               | RND 通过衡量状态的新颖性（Novelty）生成内部奖励（Intrinsic Reward），鼓励智能体探索未知状态，解决稀疏奖励问题。 | 在Actor-Critic架构的基础上，增加两个神经网络：目标网络（固定参数不训练）和预测网络，把一个状态都输入给这两个网络，他们的输出向量的MSE损失作为内部奖励。 |                         |
