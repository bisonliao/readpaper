...
| 算法名 | 动作空间  | on/off-policy | 架构         | 优劣势               | 主要思路                                                     | 探索方法        |
| ------ | --------- | ------------- | ------------ | -------------------- | ------------------------------------------------------------ | --------------- |
| MADDPG | 连续      | off           | actor-critic | 基于DDPG好理解好实现 | 基于DDPG，每个agent独立的actor/critic（Q不是V）网络。训练的时候critic的输入为全部agent的观测和动作。还提到了数据不可得的时候通过拟合其他agent的actor的方法、集合子策略提升泛化能力的方法 | actor的动作抽样 |
