...
3. 一定要小心：如果手动设置固定的目标位移，可不是随便小的绝对值都可达的，例如位移[0.1,0.1,0.1]是不可达的。我在这里浪费了一整天。reset的时候机械臂是伸直的固定位置，以咯吱窝为球心的半球是可达范围，不能再伸得更远了，所以x y z位移不能都是正的。
4. 当然环境给出的desired goal是一定可达的，只是老变化的情况下，SAC成功率不够高
5. 如果不固定desired_goal，成功率35%左右。
6. 不固定desired_goal，改为使用SB3实现的SAC，成功率可以到40%
