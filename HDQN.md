...
1. 子目标有一定的成功率（平均20%左右）
2. 最终目标也近百次的以子目标的形式出现，但作为子目标从来没有成功过。但在与环境交互的过程中，有较多次数的达成。这怎么理解呢？可能是在达成另外一个子目标的时候，达成了最终目标
...
2.  是最终目标作为子目标的成功率90%+？
3. 还是最终目标顺便完成的成功率90%+？
4. 是不是要多meta controlle做更多的分析和控制 这里保存一份[详细的子目标的活动日志](./hDQN-subgoal-activity.txt)可以再分析下
