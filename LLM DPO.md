...
DPO与RLHF不一样，采取监督学习里的二分类方法，让LLM遵从人类的偏好和控制，从而变得安全、有用、真实。它将传统的强化学习对齐过程（RLHF）转化为一个简单的监督学习问题。DPO发现，最优策略和参考策略（通常是SFT模型）之间的概率比，可以直接由人类偏好数据推导出来。因此，它不再需要训练一个独立的奖励模型（RM）再用强化学习优化，而是直接用一个分类损失函数：让模型给“好回答”的概率远高于“坏回答”。这样就绕过了不稳定的强化学习步骤，训练更简单、高效且稳定。

DPO 的监督学习样本和标注如下：

- **样本（输入）**：一个提示（prompt）和一对模型生成的回复，即 (prompt, chosen_response, rejected_response)。
- **人工标注（标签）**：人类已经标记出在这对回复中，哪一个更好（chosen / rejected）。这个偏好数据通常来自用户反馈或人工标注。
