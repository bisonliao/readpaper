...
https://arxiv.org/pdf/2303.11191

因为希望半年后我也能写一篇RL的综述，所以特意阅读一下别人的综述。这是我阅读的第一篇综述，读完后，觉得要写一篇高质量的综述还是很不容易的：

1. 要阅读非常多的论文。这篇演示学习的综述，参考了221篇论文！每一篇都在正文里引用了其关键的思路或者影响。
2. 知识面要又广又深，深刻理解他们之间的关系和区别，又要面面俱到

这篇文章没有看完，还是太泛了。

我还是要多写多输出，不一定是综述，但应该是比较系统化的一些东西，例如博客文章、PPT课件什么的
...
“演示学习”是一个框架，其中 agent 并不是通过试错学习，而是从一批“演示轨迹数据”中学习。演示可以来源于人类操作员、专家 agent，甚至是次优数据源。
...
从演示中学习本质上也是一种“悖论”。它融合了监督学习和强化学习的思路：

1. 一方面，演示学习采用监督学习方法（比如行为克隆），也就是从固定的数据中拟合策略函数，通常假设数据是 i.i.d.；
2. 另一方面，它试图完成的是一个强化学习的问题：找到一条奖励期望最大的行为路径，这恰恰要求数据是序列相关的（非 i.i.d.），并且需要探索那些演示中未曾出现过的“更优路径”。

如果你只从演示数据中学，模型永远只能“模仿”，很难“改进”；但一旦你尝试突破演示分布，又会遇到分布外状态、无法估计回报等问题。

所有这些问题本可通过和环境交互来缓解，比如用探索策略测试模型不确定的地方，因此许多方法会在初期使用演示数据训练，然后通过在线强化学习进一步微调策略。纯粹的示范学习难度较高，因为agent无法收集新的状态转移信息。

理论上，任何在线强化学习的 off-policy 方法都可以用做演示学习，但这些方法通常默认agent可以与环境交互，修正模型偏差。

纯粹的示范学习较为困难，因为 agent 无法收集新的状态转移数据，也就无法探索新区域。从技术角度讲，现有所有用于在线强化学习的 off-policy 方法理论上都可以用来从示范数据集中训练模型。然而，这些方法原本假设 agent 是可以与环境交互、从错误中修正的。示范学习中，模型是用来执行一项任务的，它基于目标状态-动作分布 Δ_task 进行估计，而示范数据集则仅覆盖了 Δ_demo ⊂ Δ_task 的一个子集。然而在实际部署中，agent 很可能会遇到落在数据集分布之外的区域 (s, a) ∉ Δ_demo。模型对这些“分布外”区域的预测错误往往更大，并会不断累积，导致 agent 行为持续偏离原本学习到的分布，这种雪球效应被称为“分布偏移”（distributional shift）。

大多数离线强化学习中的策略学习方法，会以不同方式直接处理分布偏移问题：

1. 有些方法使用行为克隆，将策略限制在数据集分布之内，但这极大地限制了泛化能力。
2. 也有方法尝试通过对不确定性的估计，在训练损失中惩罚偏离数据分布的行为；
3. 另一些方法则在 Bellman 更新中使用保守的未来奖励估计，学习值函数的下界。

文献 [43] 证明，即便拥有最优的动作标签，分布偏移带来的复合误差，在最理想情况下也会呈现出二次累积。如果允许 agent 收集额外的状态转移数据，那么误差增长则是线性的。因此，示范学习方法面临一个挑战：在提高泛化能力与避免分布偏移之间找到平衡点。

4.2 Learning Methods

![image-20250629150423232](img/image-20250629150423232.png)

#### 4.2 Policy Learning

![image-20250629152827185](img/image-20250629152827185.png)

#### 4.3 Model Learning

![image-20250629154906169](img/image-20250629154906169.png)

#### 4.4 Inverse Reinforcement Learning

![image-20250629155151004](img/image-20250629155151004.png)

#### 4.5 Other Learning Methods

![image-20250629154712359](img/image-20250629154712359.png)
