...
if __name__ == '__main__':
    ...
    writer = SummaryWriter( f'logs/racecar_expert_only_steer_{datetime.datetime.now().strftime("%m%d_%H%M%S")}')
    env = RaceCarEnv(writer, True)
    all_traj = []
    for _ in range(100):
        done = False
        ...
        if reward > 0:
            all_traj.append(traj)
            print("get a trajectory")
        print(f'trajectory len:{len(traj)}')
        print(f'has collected {len(all_traj)} trajectories')

    torch.save(all_traj, './expert_racecar_only_steer_traj.pth')
...
class Config:
    ...
    n_state, n_action, max_action = 11, 1, 1
    fps = 20
...
class TD3:
    ...
    def load2replaybuffer(self):
        traj_list = torch.load('./expert_racecar_only_steer_traj.pth', weights_only=False)
        for traj in traj_list:
            for one_step in traj:
                state, action, reward, termination, truncation, info, next_state = one_step
