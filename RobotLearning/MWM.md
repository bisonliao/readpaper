...
基于模型的强化学习（RL）通过学习一个世界模型并利用它进行规划或生成虚拟状态来进行行为学习，有望实现高效的机器人学习。然而，在具有复杂视觉观测的环境中学习世界模型，例如，准确地模拟与小物体的交互，仍然是一个开放的挑战。

我们认为这个难题来自于当前方法的设计，这些方法通常是端到端地优化世界模型，以同时学习视觉表示和动态。这在学习表示和动态之间造成了一种取舍，可能会阻碍世界模型准确捕捉视觉细节，从而让预测未来变得困难。

在这篇论文中，我们介绍了掩码世界模型（Masked World Models, MWM），这是一种visual model-based 的强化学习算法，它将视觉表示学习和动力学学习分离开来。MWM 的核心思想是训练一个自动编码器，通过卷积特征掩码重建视觉观察，然后在该自动编码器之上训练一个潜在动力学模型。通过引入早期卷积层，并掩码卷积特征而不是像素块，我们的方法使世界模型能够从复杂的视觉观察中捕捉到细粒度的视觉细节。此外，为了学习仅靠重建目标可能无法捕获的任务相关信息，我们为自动编码器引入了一个辅助奖励预测任务。具体来说，我们通过重复迭代过程分别更新视觉表示和动力学:
